...
async def get_rendered_prompt(systemPrompt: SystemPrompt) -> str:
    ...
    async for db in get_db():
        # Use systemPrompt.template to match the Pydantic model
        result = await db.execute(select(PromptTemplate).where(PromptTemplate.name == systemPrompt.template))
        db_template = result.scalars().first()

        if not db_template:
            # Handle case where the template is not found in the database
            raise ValueError(f"PromptTemplate with name '{systemPrompt.template}' not found.")
